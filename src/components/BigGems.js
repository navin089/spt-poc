        ...
        <tbody className="text-[12px] sm:text-[12px] md:text-[13px] lg:text-[14px] font-normal text-gray-700">
          ...
          {/* DATA POPULATION */}
          {
            ConfigData.data.map((item, index) => (
              ...
              { Config.children[2].visibility === true &&  <td className="tbl-cell-pt">{item.bs}</td>  }
              { Config.children[3].visibility === true &&  <td className="tbl-cell-pt">{item.target}</td>  }
              { Config.children[4].visibility === true &&  <td className="tbl-cell-pt whitespace-normal max-w-lg" style={{ width: '100%', minWidth: '400px'}}>{item.description}</td>  }
              { Config.children[5].visibility === true &&  <td className="tbl-cell-pt">{item.exitr}</td>  }
              { Config.children[6].visibility === true &&  <td className="tbl-cell-pt">{item.exitdt}</td>  }
              { Config.children[7].visibility === true &&  <td className="tbl-cell-pt">{item.disclosure}</td>  }
